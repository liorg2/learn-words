...
export class WordSearchGame extends Game {
    constructor(words, language) {
        ...
        this.wordSearchGrid = [];
        this.gridSize = 10; // Reduced grid size from 15 to 10
        this.placedWords = [];
        this.cellSize = 40; // Slightly larger cells for better readability
        this.selectedCells = [];
    ...
    canPlaceWordAt(word, startX, startY, direction) {
    ...
    placeWordAt(word, translation, startX, startY, direction) {
        const letterCells = [];
        for (let i = 0; i < word.length; i++) {
            let x, y;
            if (direction === 'horizontal') {
    ...
    getCellsBetween(firstCell, lastCell) {
        const cells = [];
        const firstX = parseInt(firstCell.dataset.x);
        const firstY = parseInt(firstCell.dataset.y);
    ...
    checkSelectedWord() {
        ...
        if (foundWord) {
            ...
            const wordItem = document.querySelector(`li[data-word="${foundWord.word}"]`);
            if (wordItem) {
